...
using RP.Sistema.Web.Models.Pais;
using RP.Util;
using System.Data;
    ...
    public class PaisController : Controller
    {
        ...
        public ActionResult Search(string filter, int? page, int? pagesize)
        {
            try
            {
                ...
                {
                    var _bll = new BLL.PaisBLL(db, _idUsuario);
        ...
        public ActionResult Report(string filter)
        {
            try
            {
                using (var db = new Context())
                {
                    return new Report.Class.Pais().GetReport(db, filter, _idUsuario);
                }
        ...
        private ActionResult GetView(int id)
        {
            try
            {
                using (var db = new Context())
                {
                    ...
                    var _pais = _bll.FindSingle(e => e.idPais == id);

                    return View(PaisVM.GetPais(_pais));
                }
            }
            catch (Exception ex)
            {
        ...
        [Auth.Class.Auth(true)]
        public JsonResult JsCreate(Pais model)
        {
            try
            {
                using (var db = new Context())
