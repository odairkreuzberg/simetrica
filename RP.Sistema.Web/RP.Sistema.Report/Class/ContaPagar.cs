...
namespace RP.Sistema.Report.Class
{
    public class ContaPagar
    {
        ...
        private DataSet GetDataSet(Model.Context db, string filter, DateTime? dtInicio, DateTime? dtFim, string situacao, int _idUsuario)
        {
            ...
            string sql = @" SELECT	idcontapagar, parcela, contapagar.descricao, vencimento, pagamento, valorconta, valorpago,  situacao, flformapagamento, nome, tipo
                            FROM	contapagar 
                            left JOIN fornecedor ON contapagar.idfornecedor = fornecedor.idfornecedor
                            left JOIN projeto ON contapagar.idprojeto = projeto.idprojeto
                           where (fornecedor.nome like'%" + filter + "%')";
        ...
        {
            private int idcontapagar { get; set; }
